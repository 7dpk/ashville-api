...
export const pointValidator = Joi.object({
    ...
    image: Joi.string().required().error(new Error('"image" is required must be a string')),
    ...
    text: Joi.string().error(new Error('text must be a string')),
    ...
    password: Joi.string().required().error(new Error('"password" is required must be a string'))
    }).required()
